...
calculate_EcM_critical <- function(species_richness_value) {
  ...
  model_data <- lme(
    Le_log ~ scale(EcM_percentage_square) + scale(EcM_percentage) +
      scale(STDAGE) + scale(SLOPE) + scale(ELEV) + 
      scale(ph) + scale(totaln) + scale(bulk) + scale(clay) +
      scale(pre) + scale(tem),
    random = ~ 1 | ECOSUBCD,
...
results <- map_dfr(2:16, calculate_EcM_critical)
print(results)
...
results_2 <- map_dfr(2:16, calculate_EcM_critical)
print(results_2)
...
p1 = ggplot(data_all, aes(x = EcM_percentage)) +
  ...
  # Add vertical reference line at 76% EcM (optional threshold)
  geom_segment(aes(x = 76, xend = 76, y = 1.5, yend = 5.3), 
...
p2 = ggplot(data_all, aes(x = EcM_percentage)) +
  ...
  # Reference line at 76% EcM (could indicate a threshold)
  geom_segment(aes(x = 76, xend = 76, y = 1.5, yend = 5.7), 
               color = "grey60", linetype = "dotted", size = 1) +
